...
<head>
<title>Listagem dos Voos para Serviços Aeroportuários</title>
...
<%
  ...
  objConn.Execute "SET DATEFORMAT ymd"
	...
	Dim intSeqUsuario, intSeqAeroporto, strNomeAeroporto, strCodAeroporto, SqlSelectAeroporto, RsAeroporto
	...
	SqlSelectAeroporto = SqlSelectAeroporto & " FROM sig_usuario SUS, sig_aeroporto SIA "
	SqlSelectAeroporto = SqlSelectAeroporto & " WHERE SUS.sequsuario=" & intSeqUsuario
	SqlSelectAeroporto = SqlSelectAeroporto & " AND RIGHT(UPPER(SUS.usuario), 3)=SIA.codiata "
	set RsAeroporto = objConn.Execute(SqlSelectAeroporto)
  ...
  if (Not isVazio(intSeqAeroporto)) then
	strSqlWhere = strSqlWhere & " AND   ( DT.seqaeroporig = " & intSeqAeroporto & " ) "
  end if
  if strDia <> "" and strMes <> "" and strAno <> "" then
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0" ID="Table1">
     <tr>
         ...
         <td class="corpo" align="center" width="30%" rowspan="2">
            <font size="4"><b>&nbsp;Serviços Aeroportuários [Horário UTC]</b></font><br>
<%
	if (Not isVazio(strNomeAeroporto) And Not isVazio(strCodAeroporto)) then
		Response.Write("<font size='4'><b>" & strNomeAeroporto & "&nbsp;(" & strCodAeroporto & ")</b></font>")
	end if
%>
         </td>
...
<center>
	...
	<tr>
		<td>
			<form method="post" action="listagemservicoaerop.asp" name="form1" id="form1" onSubmit="Javascript: return VerificarCampos();">
<%
           ' Executa função para gravar na sig_usuariolog
           If f_grava_usuariolog( "I17", objConn ) <> "" Then
              Response.End()
           End if
%>        
  ...
  If blnFazConsulta Then
  	If (Not ObjRs.Eof) Then
		Do While Not ObjRs.Eof
			...
			End If
%>
			<tr onclick="location.href ='servicoaerop.asp?seqvoodia=<%=objRs("SeqVooDia")%>&seqtrecho=<%=objRs("SeqTrecho")%>'" style='cursor:pointer;cursor:hand' >
